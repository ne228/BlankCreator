<template>
      ...
      <v-dialog class="dialog" v-model="dialog">
        <v-card>
          ...
          <v-divider></v-divider>
          <div class="text-body-2 mt-2  ml-5 font-weight-light">Выберите шаблон, чтобы вставить его в хранилище</div>
          <v-card-text>
            <!-- Список элементов -->
            <v-list v-if="items.length > 0 ">
              <v-list-item v-for="item, index in items" :key="item.id">
                ...
                <div class="text-body-2 w-100 font-weight-light">
                      {{ item.rank }}   {{ item.duty }} {{ item.dateBirth }} г.р.
                      {{ item.place }} {{ item.town }}
                    </div>     
              </v-list-item>
            </v-list>
            <div v-if=" items == 0 ">
              У вас нет шаблонов. Чтобы создать шаблон, при оздании новой справки выберите пункт "Сохранить как шаблон".
            </div>
          </v-card-text>
          <v-divider></v-divider>
